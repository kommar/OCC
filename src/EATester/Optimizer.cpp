...
template <class TGenotype, class TFenotype>
void COptimizer<TGenotype, TFenotype>::vInitialize()
{
...
void COptimizer<TGenotype, TFenotype>::vRun()
{
	...
	while (!pc_stop_condition->bStop(&c_optimizer_timer, i_iteration_number, pc_problem->pcGetEvaluation()->iGetFFE(), pc_best_individual))
	{
		bRunIteration(i_iteration_number);
		i_iteration_number++;

		d_best_fitness = pc_best_individual->dGetFitnessValue();
		c_optimizer_timer.bGetTimePassed(&d_time);
...
template <class TGenotype, class TFenotype>
bool COptimizer<TGenotype, TFenotype>::b_update_best_individual(uint32_t iIterationNumber, CIndividual<TGenotype, TFenotype> *pcIndividual)
{
	...
	if (!pc_best_individual || pc_problem->bIsBetterIndividual(pcIndividual, pc_best_individual))
	{
		...
		v_update_statistics_of_best(iIterationNumber, c_optimizer_timer.dGetTimePassed());
...
template <class TGenotype, class TFenotype>
bool COptimizer<TGenotype, TFenotype>::b_update_best_individual(uint32_t iIterationNumber, double dCurrentBestFitnessValue, function<void(TGenotype*)> &&fUpdateBestGenotype)
{
	bool b_updated = false;
...
template <class TGenotype, class TFenotype>
void COptimizer<TGenotype, TFenotype>::v_update_statistics_of_best(uint32_t iIterationNumber, double dBestTime)
{
	d_best_time = dBestTime;
