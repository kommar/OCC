...
public:
	...
	virtual double dGetNow(CTimeCounter  *pcTimeCounter, uint32_t iIterationNumber, uint64_t iFFE, CIndividual<TGenotype, TFenotype> *pcBestIndividual) = 0;
	virtual double dGetMax() = 0;

	void  vSetIgnoreOptimalAtStop(bool  bIgnoreOptimalAtStop) { b_ignore_optimal_at_stop = bIgnoreOptimalAtStop; };
	void  vSetStopNow(bool  bStopNow) { b_stop_now = bStopNow; };

private:
	CEvaluation<TFenotype> *pc_evaluation;
	bool  b_ignore_optimal_at_stop;
	bool  b_stop_now;
};//class CStopCondition
...
public:
	...
	virtual bool bStop(CTimeCounter  *pcTimeCounter, uint32_t iIterationNumber, uint64_t iFFE, CIndividual<TGenotype, TFenotype> *pcBestIndividual) { return false; }

	virtual double dGetNow(CTimeCounter  *pcTimeCounter, uint32_t iIterationNumber, uint64_t iFFE, CIndividual<TGenotype, TFenotype> *pcBestIndividual) { return STOP_CONDITION_NOW_UNSUPPORTED; }
...
public:
	...
	virtual double dGetNow(CTimeCounter  *pcTimeCounter, uint32_t iIterationNumber, uint64_t iFFE, CIndividual<TGenotype, TFenotype> *pcBestIndividual) { return (double)iIterationNumber; }
...
public:
	...
	virtual double dGetNow(CTimeCounter  *pcTimeCounter, uint32_t iIterationNumber, uint64_t iFFE, CIndividual<TGenotype, TFenotype> *pcBestIndividual) { return (double)iFFE; }
