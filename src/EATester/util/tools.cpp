...
//#define  s_DEBUG_PATH  "D:\\michal\\zz_buf\\"

#define  s_DEBUG_PATH  "C:\\Projekty\\projekty_pwr\\OptFramework\\x64\\Release\\"



CString  Tools::sFormatBool(bool  bValue)
{
	CString  s_result;
...
double  Tools::dRound(double dArg, int iPrecision)
{
	double  d_result;
	...
	else
	{
		...
		for (int ii = 0; ii < iPrecision; ii++)
			d_multiplier *= 10;

		d_result = dArg * d_multiplier;
		d_result = ::round(d_result);
		d_result = d_result / d_multiplier;
...
bool  Tools::bGetMedianAndAvr(vector<int>  *pvTable, bool bSortTable, double  *pdMed,  double  *pdAvr, int *piMin, int *piMax)
{
	double  d_avr, d_med;
	...
	*pdAvr = 0;
	...
	if (bSortTable == true)  std::sort(pvTable->begin(), pvTable->end(), [](const int iLen0, const int iLen1) -> bool { return(iLen0 < iLen1); });

	if ((piMin != NULL) && (pvTable->size() > 0))  *piMin = pvTable->at(0);
	if ((piMax != NULL) && (pvTable->size() > 0))  *piMax = pvTable->at(0);

	d_avr = 0;
	for (int ii = 0; ii < pvTable->size(); ii++)
	{
		d_avr += pvTable->at(ii);
		if (piMin != NULL) { if (*piMin > pvTable->at(ii))  *piMin = pvTable->at(ii); }
		if (piMax != NULL) { if (*piMax < pvTable->at(ii))  *piMax = pvTable->at(ii); }

		if (bSortTable == false)
		{
			if (ii > 0)
			{
				if (pvTable->at(ii) < pvTable->at(ii - 1))  return(false);
			}//if (ii > 0)
		}//if  (bSortTable == false)

	}//for (int ii = 0; ii < v_tlbs.size(); ii++)
	d_avr = d_avr / pvTable->size();

	
	d_med = 0;
	if (pvTable->size() > 0)
	{
		if ((pvTable->size() / 2) * 2 < pvTable->size())
			d_med = pvTable->at(pvTable->size() / 2);
		else
		{
			d_med = pvTable->at(pvTable->size() / 2) + pvTable->at(pvTable->size() / 2 - 1);
			d_med = d_med / 2;
		}//else  if ((v_tlbs_lens.size() / 2) * 2 < v_tlbs_lens.size())
	}//if (v_tlbs_lens.size() > 0)

	*pdMed = d_med;
	*pdAvr = d_avr;

	return(true);
}//bool  Tools::bGetMedianAndAvrWithSORTING(vector<int>  *pvTable)
