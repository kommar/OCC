...
int Benchmarks::getSSize() {
	return s_size;
}


vector<int>& Benchmarks::getFuncSizes() {
	return funcSizes;
}

vector<int>& Benchmarks::getVariablesForFunc(int idx) {
	return variablesForFunc[idx];
}

vector<int>& Benchmarks::getFuncsForVariable(int idx) {
	return funcForVariables[idx];
}

unordered_set<int>& Benchmarks::getVariablesForVariable(int idx) {
	return variablesForVariables[idx];
}
...
double Benchmarks::rosenbrock_(double*x, int dim, int offset) {
	int    j;
	double oz, t;
	double s = 0.0;
	j = dim - 1;
