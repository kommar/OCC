...
class Benchmarks{
 protected:
  ...
  double rosenbrock_(double*x, int dim, int offset);
  ...
  vector<int> funcSizes;
  vector<vector<int>> variablesForFunc;
  vector<vector<int>> funcForVariables;
  vector<unordered_set<int>> variablesForVariables;
 ...
 public:
  ...
  virtual double compute(double* x, int s_idx){return 0;};
  ...
  vector<int>& getVariablesForFunc(int idx);
  vector<int>& getFuncsForVariable(int idx);
  unordered_set<int>& getVariablesForVariable(int idx);
